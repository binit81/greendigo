...
@foreach($category AS $category_key=>$category_value)
    ...
    ?>
    <tr id="{{$category_value->category_id}}" class="<?php echo $tblclass ?>">
        <td>
        <?php 
        if($role_permissions['permission_delete']==1)
        {
        ?>
            <input type="checkbox" name="delete_source[]" value="{{$category_value->customer_source_id }}" id="delete_source{{$category_value->customer_source_id }}">
        <?php
        }
        ?>
        ...
        {
        ?>
            <a onclick="return editsource('{{encrypt($category_value->customer_source_id)}}');">
            <i class="fa fa-edit" title="Edit Customer Source"></i></a>
        <?php
        }
        ?>
        </td>
        <td class="leftAlign">{{$category_value->category_name}}</td>
        <!-- <td class="leftAlign">{{$category_value->note}}</td> -->
    </tr>
@endforeach
...
<script type="text/javascript">
    $(".PagecountResult").html(' ({{$category->total()}})');
</script>
